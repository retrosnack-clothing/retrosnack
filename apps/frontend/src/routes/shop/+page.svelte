...
<section class="mx-auto max-w-6xl px-4 py-12">
  <div class="flex items-center justify-between mb-6">
    ...
    {#if !loading && !error}
      <span class="text-sm text-ink-muted">{filtered.length} item{filtered.length !== 1 ? 's' : ''}</span>
    {/if}
  </div>
  ...
  {:else}
    <div class="space-y-5 mb-8">
      <!-- search -->
      <div class="relative">
        <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-muted" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="px-3.5 py-1.5 rounded-full text-sm border transition-colors {activeCategory === 'all'
          ...
          {#each categories as cat (cat.id)}
            <button
              ...
              class="px-3.5 py-1.5 rounded-full text-sm border transition-colors {activeCategory === cat.id
                ? 'border-ink bg-ink text-sand'
                : 'border-border hover:border-ink'}"
            >
              {cat.name}
    ...
    {#if filtered.length === 0}
      <p class="text-center text-ink-muted py-16">no items match your search — try something else.</p>
    {:else}
      <ProductGrid>
